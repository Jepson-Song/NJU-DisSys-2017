...
func TestInitialElection(t *testing.T) {
	//log.Printf("test->TestInitialElection")
	servers := 3
...
func TestReElection(t *testing.T) {
	...
	//fmt.Printf("Test: if the leader disconnects, a new one should be elected. ...\n")
	cfg.disconnect(leader1)
	cfg.checkOneLeader()

	// if the old leader rejoins, that shouldn't
	// disturb the old leader.
	//fmt.Printf("Test: if the old leader rejoins, that shouldn't disturb the old leader. ...\n")
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	//fmt.Printf("Test: if there's no quorum, no leader should be elected. ...\n")
	...
	//fmt.Printf("Test: if a quorum arises, it should elect a leader. ...\n")
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()

	// re-join of last node shouldn't prevent leader from existing.
	//fmt.Printf("Test: re-join of last node shouldn't prevent leader from existing. ...\n")
	cfg.connect(leader2)
	cfg.checkOneLeader()
